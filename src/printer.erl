...
pprint(#invoice{type=Type, doc_no=DocNo, bpartner=BPartner,
                trx_ts=Ts, discount=Discount, total=Total, lines=Lines}) ->
    io:fwrite("~n[~10s] [~-15s] [~-20s] [~25s]~n",
              [Type, DocNo, BPartner, Ts]),
    SortedLines =
        lists:sort(fun(#invoice_line{line_no=LineNo1},
...
pprint(#invoice_line{line_no=LineNo, product=Product, qty=Qty,
    ...
    io:fwrite("~4B ~-20s ~6B ~10.2f ~12.2f~n",
