...
process(Line, Invoices) ->
    ...
    case dict:find(DocNo, Invoices) of
        ...
        _ ->
            ...
            dict:store(DocNo, Invoice#invoice{ lines = [InvoiceLine] }, Invoices)
    end.
...
new_invoice(DocNo, Customer, Date, Discount, Total) ->
    try
        ...
        error:badarg -> throw({invalid_invoice, {DocNo, Customer, Date, Discount, Total}})
    end.
...
new_invoice_line(LineNo, Product, Qty, Price, LineAmt) ->
    try
        ...
        error:badarg -> throw({invalid_invoice_line, {LineNo, Product, Qty, Price, LineAmt}})
    end.
...
pprint(#invoice{ doc_no = DocNo, customer = Customer, date = Date, discount = Discount, total = Total, lines = Lines }) ->
    ...
    SortedLines = lists:sort(
                    fun(#invoice_line{ line_no = LineNo1 }, #invoice_line{ line_no = LineNo2 }) ->
                            LineNo1 =< LineNo2
                    end,
                    Lines),
