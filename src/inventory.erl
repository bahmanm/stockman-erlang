...
%% @private
%%---
-spec handle_call({move_in, {Product :: product(), Qty :: integer()}},
                  From :: any(), State :: state()) ->
          {reply, ok, NewState :: state()};
                 ({move_out, {Product :: product(), Qty :: integer()}},
                  From :: any(), State :: state()) ->
          {reply, ok | {error, insufficient_inventory | unknown_product},
           NewState ::state()};
                 ({available_qty,  Product :: product()},
                  From :: any(), State :: state()) ->
          {reply, {ok, Qty :: integer()} | {error, unknown_product},
           NewState :: state()}.

handle_call({move_in, {P, Q}}, _, #state{items=Items}=S) ->
    Items1 = case dict:is_key(P, Items) of
                 true ->
                     Items;
                 _ ->
                     dict:store(P, #inventory{product=P, qty=0},
                                Items)
             end,
