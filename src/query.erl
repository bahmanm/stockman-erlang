...
most_expensive_product(
    [#invoice_line{product = Product, price = Price} | Lines],
...
avg_price_per_product(Invoices) ->
    ...
    AvgDict = dict:map(
        fun(_, {Qty, TotalQtyxPrice}) ->
            TotalQtyxPrice / Qty
        end,
        QtyAndTotalDict
    ),
    AvgList = dict:to_list(AvgDict),
    SortedAvgList = lists:sort(
        fun({_, AvgA}, {_, AvgB}) ->
            AvgA > AvgB
        end,
        AvgList
...
avg_price_per_product(
    [#invoice_line{product = Product, price = Price, qty = Qty} | Lines],
    Result
) ->
    CurrentResult = dict:update(
        Product,
        fun({RunningQty, RunningQtyxPrice}) ->
            {Qty + RunningQty, RunningQtyxPrice + (Qty * Price)}
        end,
        {Qty, Qty * Price},
        Result
    ),
...
total_sales_per_customer_dict(Invoices) ->
    dict:fold(
        fun(_, #invoice{bpartner = Customer, total = InvoiceTotal}, Result) ->
            dict:update(
                Customer,
...
date_with_largest_total_sales(Invoices) ->
    DateTotals = dict:fold(
        fun(_, #invoice{trx_ts = Ts, total = InvoiceTotal}, Result) ->
    ...
    case dict:to_list(DateTotals) of
        ...
        [Elem1 | T] ->
            lists:foldl(
                fun({_, Total1} = Elem, {_, Total2} = Acc) ->
                    if
                        Total1 > Total2 ->
                            Elem;
                        true ->
...
most_expensive_product_test() ->
    ...
    Invoices = dict:from_list(
        [
            {"i1", #invoice{
                lines = [
                    #invoice_line{product = "P1", price = 10.0},
                    #invoice_line{product = "P2", price = 8.5}
                ]
            }},
            {"i2", #invoice{
                lines = [
                    #invoice_line{product = "P3", price = 1.0},
...
avg_price_per_product_test() ->
    ...
    Invoices = dict:from_list(
        [
            {"i1", #invoice{
                lines = [
                    #invoice_line{product = "P1", price = 10.0, qty = 3},
                    #invoice_line{product = "P2", price = 10.0, qty = 5}
                ]
            }},
            {"i2", #invoice{
                lines = [
                    #invoice_line{product = "P2", price = 12.0, qty = 3},
